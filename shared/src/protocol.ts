...
/** Select which Exocortex daemon the local daemon socket routes to. */
export interface SshCommand {
  type: "ssh";
  reqId?: string;
  action: "connect" | "status" | "cancel";
  /** Required for connect. This is passed as one destination argv to ssh. */
  alias?: string;
}
...
/** Local routing state. This event is generated by the local daemon, never the remote one. */
export interface SshStatusEvent {
  ...
  alias?: string;
  /** True only when a successful command changed the selected daemon. */
  switched: boolean;
