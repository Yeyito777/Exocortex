...
export const QUEUE_COMMAND: SlashCommand = {
  ...
  handler: (_text, state) => {
    pushSystemMessage(state, "Usage: include /queue in a message to send it after all TUI conversations and queued turns are idle. Use /queue <conversation-or-folder> to wait for one conversation, or a 📁 folder, instead.");
    clearPrompt(state);
    return { type: "handled" };
  },
};
