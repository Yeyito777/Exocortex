...
describe("sidebar rendering", () => {
  ...
  test("renders goal continuations with the ordinary streaming indicator", () => {
    const sidebar = createSidebarState();
    sidebar.folders = [
    ...
    rows = renderSidebar(sidebar, 8, true, null);
    expect(rows.find(row => row.includes("Reviewing goal"))).toContain(`${theme.accent}◉ `);
  });
  ...
  test("renders durable Chrono sleeps with the yellow streaming indicator", () => {
    ...
    sidebar.conversations = [
      conversation("deferred-sleep", 0, {
        ...
        folderId: "folder",
        unread: true,
        tasks: [{ id: "chrono-long-sleep", kind: "chrono", title: "Sleep for ten minutes", startedAt: 0, dueAt: Date.now() + 10 * 60_000, chronoMode: "sleep" }],
      }),
    ...
    expect(rows.find(row => row.includes("Work"))).not.toContain(`${theme.success}◉ `);
