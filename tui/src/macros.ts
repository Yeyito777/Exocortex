...
const AUTORESEARCH_PROMPT = "You're going to autoresearch. Set yourself a goal in accordance with the topic, and when you set it use pausable=false and completable=false so the goal is unpausable and uncompletable. Make a gitignored directory in the project called \"autoresearch/<topic>\" for the raw benchmark, experiment outputs, and success/failure ledger. Never force-add or commit this directory. To know which experiments to keep or trash, you must create the benchmark first and make every experiment deterministic against it. On success, commit only the accepted production source, tests, and durable documentation outside the autoresearch directory. On failure, revert, stash, or delete the failed production change after recording the result in the local ledger. This lets repeated experiments improve the benchmark without accumulating generated research artifacts in the repository. Make sure to not use subagents. With all that said, this is the user request to autoresearch; choose how to interpret it as a benchmark and how to begin the research direction. Make sure to ask him 5 questions before you actually start, and set yourself the goal AFTER the user has answered the five questions:";
