...
async function startFakeDaemon(socketPath: string): Promise<FakeDaemon> {
  ...
  const writeEvent = (socket: Socket, event: Event) => socket.write(`${JSON.stringify(event)}\n`);

  const server = createServer((socket) => {
    sockets.add(socket);
    socket.setEncoding("utf8");
    writeEvent(socket, toolsAvailableEvent());
    writeEvent(socket, { type: "conversations_list", conversations: [], folders: [] });
    ...
    socket.on("data", (chunk) => {
      ...
      while ((idx = lineBuffer.indexOf("\n")) !== -1) {
        ...
        if (command.type === "ping") writeEvent(socket, { type: "pong" });
      }
    });
...
describe("voice recall real TUI flow", () => {
  ...
  test("Enter-recalling a submitted transcription ignores the Enter release and keeps completion in the prompt", async () => {
    ...
    const fakeDaemon = await startFakeDaemon(socketPath);
    ...
    await waitFor(() => fakeDaemon.commands.some(command => command.type === "ping"));
    ...
    const transcriptionCommand = await waitFor(() => fakeDaemon.commands.find(isTranscribeAudioCommand));
    proc.stdin.write("\r");
    await delay(120);
    ...
    fakeDaemon.broadcast({ type: "transcription_result", reqId: transcriptionCommand.reqId, text: "recalled transcript" });
    await delay(250);
