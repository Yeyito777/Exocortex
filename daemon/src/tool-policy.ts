...
export async function applyToolPolicyMutation(
  ...
  try {
    if (modulePaths.length > 0 && mutation.action === "enable") {
      const requestedPaths = await Promise.all(
        modulePaths.map((path) => canonicalizeCustomToolModulePath(path, true, workingDirectory)),
      );
      ...
      const requestedPaths = new Set(await Promise.all(
        modulePaths.map((path) => canonicalizeCustomToolModulePath(path, false, workingDirectory)),
      ));
      ...
      await restoreLoadedModules(conversation, modules, workingDirectory);
      changedLoadedModules = true;
    }
    ...
    if (changedLoadedModules) {
      await restoreLoadedModules(conversation, previousModules, workingDirectory).catch(() => {});
    }
    throw error;
  }
}
