...
describe("conversation-scoped custom tools", () => {
  test("resolves relative modules and factory context from the conversation workspace", async () => {
    const path = await temporaryModule(`
      export default ({ workingDirectory }) => ({
        tools: [{
          name: "workspace_probe",
          description: "Report the custom tool workspace",
          inputSchema: { type: "object", properties: {}, additionalProperties: false },
          display: { label: "Workspace Probe", color: "#12abef" },
          summarize() { return { label: "Workspace Probe" }; },
          async execute() { return { output: workingDirectory, isError: false }; },
        }],
      });
    `);
    const workingDirectory = join(path, "..");
    const conversation = createConversation("custom-workspace", "openai", "gpt-5.6-sol");
    conversationIds.push(conversation.id);

    conversation.toolPolicy = await applyToolPolicyMutation(conversation, {
      action: "enable",
      tools: [],
      modulePaths: ["toolset.ts"],
    }, workingDirectory);
    ...
    const [result] = await buildExecutor(
      ...
      ["workspace_probe"],
    )([{ id: "workspace-probe", name: "workspace_probe", input: {} }]);
