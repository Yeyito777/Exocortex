...
export async function canonicalizeCustomToolModulePath(
  path: string,
  mustExist = true,
  ...
  const expanded = requested === "~"
    ...
    : requested.startsWith("~/") || (process.platform === "win32" && requested.startsWith("~\\"))
      ...
      : requested;
  const absolute = resolve(workingDirectory, expanded);
...
async function loadModule(
  ...
  const path = await canonicalizeCustomToolModulePath(request.path, true, workingDirectory);
...
export async function installConversationCustomToolModules(
  ...
  return withConversationUpdate(conversationId, async () => {
    ...
    for (const request of requests) {
      const canonical = await canonicalizeCustomToolModulePath(request.path, true, workingDirectory);
    ...
    try {
      for (const request of normalizedRequests) staged.push(await loadModule(conversationId, request, workingDirectory));
