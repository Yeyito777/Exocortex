 ...
 *
 * Periodically scans active streams and interrupts any that have been
 ...
 *
 * During a provider request the abort fires a replaceable attempt controller,
 * allowing the ordinary retry path to recover. Outside a retryable provider
 * request it still reaches the root turn controller and triggers normal cleanup.
 */
...
export function startWatchdog(): void {
  ...
  timer = setInterval(() => {
    ...
    for (const [convId, ac, inactiveMs] of stale) {
      ...
      log("warn", `watchdog: interrupting stale stream for ${convId} (inactive ${inactiveSec}s, threshold ${STALE_STREAM_TIMEOUT / 1000}s)`);
      ac.abort("watchdog");
    }
  }, CHECK_INTERVAL);
